...
bt_id_to_return = [
    'return', 'win_rate', 'worst_trade',
    'max_trade_duration', 'avg_trade_duration']
bt_var_to_return = [
    'Return [%]', 'Win Rate [%]', 'Worst Trade [%]',
    'Max. Trade Duration', 'Avg. Trade Duration']

app.layout = html.Div(
    children=[
        html.H1("Stock Market Backtesting Web Page"),
        html.Div(
            children=[
                html.Div(
                    style={
                    ...
                    children=[
                        ...
                        html.H4("Amount to invest:", style={
                            'textAlign': 'center'}),
                        dcc.Slider(
                            id="amount_invested",
                            min=0,
                        ...
                        html.Div(
                            style={
                                'width': '30%', 'border': '1px solid #73AD21',
                                'paddingLeft': '1%', 'float': 'center',
                                'display': 'inline-block'},
                            children=[
                                html.H5(
                                    "Data Frequency:",
                                ...
                                dcc.RadioItems(
                                    id="data_frequency",
                                    options=[
                                        {'label': 'Every 1 minute (last 7 days)',
                                         'value': '1m'},
                                        {'label': 'Every 1 hour (last 730 days)',
                                         'value': '1h'},
                                        {'label': 'Every 1 day (all historical data)',
                                         'value': '1d'}
                                    ],
                                    value='1h',
                                    persistence=True,
                        ...
                        html.Div(
                            style={'width': '25%', 'display': 'inline-block',
                                   'float': 'center', 'paddingLeft': '1%'},
                            children=[
                                html.H5(
                                    ["Start Date:", html.Br(), "End Date:"],
                                    style={
                                        'display': 'inline-block',
                                        'textAlign': 'center',
                                        'width': '40%'}),
                                dcc.DatePickerRange(
                                    id="invest_date",
                                    start_date=dt.date(
                                        dt.today()-datetime.timedelta(730)),
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                ...
                                html.P(id={'type': var, 'index': stock})
                                for var in bt_id_to_return
                            ]
                        ),
                        html.Div(
                            style={'width': '49%', 'textAlign': 'center',
...
@app.callback(
    [
        ...
        Output({'type': 'bt_graph', 'index': MATCH}, 'figure'),
            ...
            Output({'type': var, 'index': MATCH}, 'children')
            for var in bt_id_to_return
        ],
    [
...
def div_content_callback(stock_name: str, df_json, amount_invested: int,
                         N1: int, N2: int):
    ...
    bt_max_trd_duration = f"Max. Trade Duration: \
        {output['Max. Trade Duration']}"
    bt_avg_trd_duration = f"Avg. Trade Duration: \
        {output['Avg. Trade Duration']}"
    ...
    return figure, profit_percentage, profit_amount, time_until_sell,\
        bt_figure, bt_return, bt_win_rate, bt_wrst_trd, \
        bt_max_trd_duration, bt_avg_trd_duration
...
def get_data(stock_name: str, start_date: str,
             end_date: str, data_frequency: str):
    return pd.read_csv(
        f'/data/5stocks-{data_frequency}.csv',
        parse_dates=True,
        index_col=0,
        header=[0, 1])[stock_name].dropna().loc[
            datetime.datetime.strptime(
